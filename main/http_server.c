...
// esp_err_t long_async(httpd_req_t *req)
// {
//     char *buf;
//     size_t buf_len;
...
// esp_err_t quick_handler(httpd_req_t *req)
// {
...
{
    ESP_LOGI(TAG, "uri: /");
    const char *welcome_msg = "<h1>Welcome to ESP Mesh Intelligence!</h1>"
                              "<p>This is a simple web server running on ESP32.</p>"
                              "<ul>"
                              "<li><a href=\"/mesh\">Show Mesh Tree</a></li>"
                              "</ul>";
    httpd_resp_sendstr(req, welcome_msg);
    return ESP_OK;
}
